import RouteTraceShared
import SwiftUI

struct RouteShapeThumbnail: View {
    let routePoints: [RoutePoint]
    let boundingBox: GeoBoundingBox
    var size: CGFloat = 40

    var body: some View {
        Canvas { context, canvasSize in
            guard routePoints.count >= 2 else { return }

            let inset: CGFloat = 3
            let drawRect = CGRect(
                x: inset,
                y: inset,
                width: canvasSize.width - inset * 2,
                height: canvasSize.height - inset * 2
            )

            var path = Path()
            for (index, point) in routePoints.enumerated() {
                let nx = (point.longitude - boundingBox.minLongitude)
                    / max(0.0001, boundingBox.maxLongitude - boundingBox.minLongitude)
                let ny = 1 - (point.latitude - boundingBox.minLatitude)
                    / max(0.0001, boundingBox.maxLatitude - boundingBox.minLatitude)
                let pt = CGPoint(
                    x: drawRect.minX + drawRect.width * nx,
                    y: drawRect.minY + drawRect.height * ny
                )
                if index == 0 {
                    path.move(to: pt)
                } else {
                    path.addLine(to: pt)
                }
            }

            context.stroke(
                path,
                with: .color(.blue),
                style: StrokeStyle(lineWidth: 2, lineCap: .round, lineJoin: .round)
            )
        }
        .frame(width: size, height: size)
        .background(Color.secondary.opacity(0.12), in: RoundedRectangle(cornerRadius: 8))
    }
}

struct RouteShapeThumbnailLoader: View {
    let route: RouteEntity

    @EnvironmentObject private var routeStore: RouteStore
    @State private var routePoints: [RoutePoint] = []

    var body: some View {
        RouteShapeThumbnail(
            routePoints: routePoints,
            boundingBox: route.boundingBox
        )
        .task(id: route.id) {
            routePoints = (try? routeStore.loadRoutePackage(for: route))?.route ?? []
        }
    }
}
